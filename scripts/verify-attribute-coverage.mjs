...
async function main() {
  ...
  for (const filename of yearFiles) {
    ...
    const tagged = payload.works.filter((work) => {
      validateEmbedded(work, String(work.work_id));
      return true;
    }).length;
    if (tagged !== payload.works.length) fail(`${payload.year}: embedded attribute coverage is incomplete.`);
    ...
    byYear[String(payload.year)] = { tagged_count: tagged, total_count: payload.works.length, coverage_ratio: 1 };
  }
