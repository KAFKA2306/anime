...
test('manga publication credit is not mislabeled as web novel', () => {
  ...
  assert.equal(result.rule_id, 'origin.publication-credit-manga');
});

test('high fantasy requires official fantasy genre plus explicit world signal', () => {
  const positive = inferPrimaryGenre({
    officialGenres: ['SF/ファンタジー', 'アクション/バトル'],
    synopsis: '異世界に召喚された冒険者が魔王に挑む。',
  });
  assert.equal(positive.value, '異世界・ハイファンタジー');
  assert.equal(positive.confidence, 'derived');

  const negative = inferPrimaryGenre({
    officialGenres: ['SF/ファンタジー', '恋愛/ラブコメ'],
    synopsis: '芸能界で真相を追う兄妹の物語。',
  });
  assert.equal(negative.value, 'SF・ファンタジー');
  assert.equal(negative.confidence, 'verified');
});

test('action official genre becomes canonical battle/action tag', () => {
...
test('original credit extraction stops at next staff separator', () => {
  assert.equal(
    extractOriginalCredit('原作:作者名（電撃文庫刊）／監督:監督名／音楽:作曲家'),
    '作者名(電撃文庫刊)',
  );
});

test('official detail text parser extracts auditable metadata', () => {
